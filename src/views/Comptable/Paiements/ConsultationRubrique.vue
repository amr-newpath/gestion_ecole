    ...
    <div class="flex flex-col justify-center h-full">
      ...
      >
        <header class="px-5 py-4 border-b border-gray-100">
          <h2 class="font-semibold text-gray-800">Consultation rubrique</h2>
        </header>
        <div class="p-3">
            ...
            <table class="table-auto w-full" v-if="!isLoading">
              ...
              <tbody class="text-sm divide-y divide-gray-100">
                <tr v-for="(eleve, index) in filteredEleves" :key="index">
                  ...
                  <td class="p-4 whitespace-nowrap">
                    <div class="text-left">
                      {{ calculateServicesMontant(eleve) }} DHs
                    </div>
                  </td>
                  <td class="p-4 whitespace-nowrap">
...
export default {
  ...
  computed: {
    ...
    calculateTotalMontant() {
      ...
      const total = this.filteredEleves.reduce(
        (acc, eleve) => acc + this.calculateServicesMontant(eleve),
        0,
      );
      return total + " DHs";
    },
  ...
  methods: {
    ...
    calculateServicesMontant(eleve) {
      return eleve.paiements.reduce((totalMontant, paiement) => {
        return totalMontant + this.calculateUnpaidServicesMontant(paiement.services);
      }, 0);
    },
    calculateUnpaidServicesMontant(services) {
      return services.reduce((totalMontant, service) => {
        if (
          (service.service === "Frais scolarité" || service.service === "Transport") &&
          service.pivot.paid === 0 &&
          (!this.selectedMonth || service.pivot.month === this.selectedMonth)
        ) {
          totalMontant += service.pivot.price;
        }
        return totalMontant;
      }, 0);
    },
