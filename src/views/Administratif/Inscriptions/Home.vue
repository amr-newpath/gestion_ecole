          ...
          <div id="modal-group-1" uk-modal>
            <form method="post" @submit.prevent="addFamile" class="uk-modal-dialog">
              ...
              <div class="uk-modal-body">
                ...
                <div class="w-full px-3 mb-4 sm:h-1/2">
                  <label for="situation" class="block text-sm font-medium text-gray-600 mb-2">Situation :</label>
            ...
            <div class="uk-modal-dialog">
              ...
              <div class="uk-modal-body overflow-x-auto bg-white dark:bg-neutral-700" style="height: 60vh;">
                ...
                <div class="" v-if="id_famile != null">
                  <ParentComponents class="my-5" v-show="div1" />
                  <ElevesComponents class="my-5" v-show="div2" />
                  <SouscriptionComponent class="my-5" v-show="div3" />
                </div>
            ...
            <tbody>
              <tr v-for="(famille, index) in displayedItems" :key="index"
                ...
                <td class="px-2 text-center dark:border-neutral-600">
                  <div class="row" v-for="(parent, index) in famille.parente" :key="index"><span class="py-2"
                      v-show="index < 3">{{ parent.nom }}</span></div>
                </td>
                <td class="px-6 text-center border-x dark:border-neutral-600">
                  <div class="row" v-for="(parent, index) in famille.parente" :key="index"><span class="py-2"
                      v-show="index < 3">{{ parent.liendeparente.type }}</span></div>
                </td>
                <td class="px-6 text-center border-x dark:border-neutral-600">
                  <div class="row" v-for="(eleve, index1) in famille.eleves" :key="index1"><span class="py-2"
                      v-show="index1 < 3">{{ eleve.nom }}</span></div>
                </td>
                <td class="px-6 text-center border-x dark:border-neutral-600">
                  <div class="row" v-for="(eleve, index1) in famille.eleves" :key="index1"><span class="py-2"
                      v-show="index1 < 3">{{ eleve.niveau }}</span></div>
                </td>
                <td class=" text-center border-x dark:border-neutral-600">
                  <router-link class="btn text-info mr-1"
                    :to="{ name: 'MoreInfoInscription', params: { id: famille.id } }">
...
export default {
  ...
  methods: {
    cleanid() {
      localStorage.removeItem('id_famile');
    },
    ...
    async addFamile() {
      ...
      try {
        ...
        localStorage.setItem("id_famile", response.data.data.famille.id);
        this.famille = {
          nom: '',
          situation: 'Marié',
        };
